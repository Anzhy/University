...
int main(int *argc, char **argv)
{
	...
	ip *pIpHeader;		// Указатель на структуру-заголовок IP
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	sockAddrLocal.sin_addr = *((LPIN_ADDR) *lpHostEntry->h_addr_list); // Пишим адрес из lpHostEntry в sockAddrLocal
	if ((hSocket = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP)) == INVALID_SOCKET) // Создаем RAW сокет и указываем ICMP в качестве протокола
	{
		printf("error socket\n");
		return 0;
	}
	...
	*pdwTimeStamp = GetTickCount();
	...
	pIcmpHeader = (struct icmp *) (IcmpRecvPacket + 20);//iIPHeadLength);
	
	//// Проверяем, что мы приняли именно "эхо"-ответ
	if (pIcmpHeader->icmp_type != ICMP_ECHOREPLY) 
	{
		printf("Received packet was not an echo reply to your ping\n");
		return 0;
	}
	printf("Round-trip travel time (RTT) to %s [%s] was %d ms\n",(LPSTR)argv[1], (LPSTR)inet_ntoa(sockAddrHost.sin_addr), dwRoundTrip);
	getch();
	return(1);
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
